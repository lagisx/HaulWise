...
public class ForgetPasswordController {
    ...
    private void verifyOtp(ActionEvent event) {
        ...
        if (code.isEmpty()) {
            showStatus("Введите код из письма", "#dc2626");
            return;
        }
        if (resolvedEmail == null) {
            showStatus("Сначала отправьте код", "#dc2626");
            return;
        }
    ...
    private void changePassword(ActionEvent event) {
        ...
        if (newPass.isEmpty()) {
            showStatus("Введите новый пароль", "#dc2626");
            return;
        }
        if (newPass.length() < 6) {
            showStatus("Минимум 6 символов", "#dc2626");
            return;
        }
        if (!newPass.equals(confirmPass)) {
            showStatus("Пароли не совпадают", "#dc2626");
            return;
        }
